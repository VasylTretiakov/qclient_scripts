...
# download binaries
curl -O https://releases.quilibrium.com/node-2.0.1-b1-testnet-linux-amd64
curl -O https://releases.quilibrium.com/qclient-2.0.1-testnet-linux-amd64
chmod +x *-linux-amd64

# shortcuts
testnet-node() { ./node-2.0.1-b1-testnet-linux-amd64 --signature-check=false --network=1 $*; }
testnet-qclient() {
